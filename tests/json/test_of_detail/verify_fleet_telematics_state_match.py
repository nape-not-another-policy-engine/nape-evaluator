def evaluate(evidence, evaluations, metadata):
    ...
    match_evaluation = evaluation_index.get("telematics_state_match")
    if match_evaluation is None:
        return _build_inconclusive_result(
            "This test requires a telematics_state_match evaluation with an equals criterion."
        )

    expected_value = _read_boolean_equals(match_evaluation)
    if expected_value is None:
        return _build_inconclusive_result(
            "This test requires telematics_state_match criteria.equals to be boolean."
        )

    match_fact = _extract_state_match_fact(evidence)
    if match_fact["status"] != "found":
        return _build_inconclusive_result(
            [match_fact],
            "Unable to evaluate because the telematics_state_match fact could not be established.",
        )
...
def _evaluate_equals(match_fact, expected_value):
    ...
    if actual == expected_value:
        return {
            ...
            "facts": [match_fact],
            "reason": (
                f"telematics_state_match is {actual}, which matches the expected value."
            ),
        }
    return {
        "conclusion": "false",
        "facts": [match_fact],
        "reason": (
            f"telematics_state_match is {actual}, which does not match the expected value {expected_value}."
        ),
    }
