def evaluate(evidence, evaluations, metadata):
    ...
    count_evaluation = evaluation_index.get("internet_exposed_controller_count")
    if count_evaluation is None:
        return _build_inconclusive_result(
            "This test requires an internet_exposed_controller_count evaluation with a maximum criterion."
        )

    maximum = _read_integer_criterion(count_evaluation, "maximum")
    if maximum is None:
        return _build_inconclusive_result(
            "This test requires internet_exposed_controller_count criteria.maximum to be an integer."
        )
    ...
    if count_fact["status"] != "found":
        return _build_inconclusive_result(
            [count_fact],
            "Unable to evaluate because the internet_exposed_controller_count fact could not be established.",
        )
...
def _evaluate_maximum(count_fact, maximum):
    ...
    if actual <= maximum:
        return {
            ...
            "facts": [count_fact],
            "reason": (
                f"internet_exposed_controller_count is {actual}, which is within the allowed maximum of {maximum}."
            ),
        }
    return {
        "conclusion": "false",
        "facts": [count_fact],
        "reason": (
            f"internet_exposed_controller_count is {actual}, which exceeds the allowed maximum of {maximum}."
        ),
    }
