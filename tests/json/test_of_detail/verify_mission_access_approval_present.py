def evaluate(evidence, evaluations, metadata):
    ...
    approval_evaluation = evaluation_index.get("mission_access_approval_present")
    if approval_evaluation is None:
        return _build_inconclusive_result(
            "This test requires a mission_access_approval_present evaluation with an equals criterion."
        )

    expected_value = _read_boolean_equals(approval_evaluation)
    if expected_value is None:
        return _build_inconclusive_result(
            "This test requires mission_access_approval_present criteria.equals to be boolean."
        )
    ...
    if approval_fact["status"] != "found":
        return _build_inconclusive_result(
            [approval_fact],
            "Unable to evaluate because the mission_access_approval_present fact could not be established.",
        )
...
def _evaluate_equals(approval_fact, expected_value):
    ...
    if actual == expected_value:
        return {
            ...
            "facts": [approval_fact],
            "reason": (
                f"mission_access_approval_present is {actual}, which matches the expected value."
            ),
        }
    return {
        "conclusion": "false",
        "facts": [approval_fact],
        "reason": (
            f"mission_access_approval_present is {actual}, which does not match the expected value {expected_value}."
        ),
    }
