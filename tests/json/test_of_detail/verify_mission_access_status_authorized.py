def evaluate(evidence, evaluations, metadata):
    ...
    status_evaluation = evaluation_index.get("mission_access_status")
    if status_evaluation is None:
        return _build_inconclusive_result(
            "This test requires a mission_access_status evaluation with an equals criterion."
        )

    expected_status = _read_expected_status(status_evaluation)
    if expected_status is None:
        return _build_inconclusive_result(
            "This test requires mission_access_status criteria.equals to be a text value."
        )
    ...
    if status_fact["status"] != "found":
        return {
            ...
            "facts": [status_fact],
            "reason": "The mission access evidence did not provide a usable mission access status.",
        }
...
def _evaluate_status(status_fact, expected_status):
    ...
    if actual_status == expected_status:
        return {
            "conclusion": "true",
            "facts": [status_fact],
            "reason": f"Mission access status is {expected_status}.",
        }
    ...
    return {
        ...
        "facts": [status_fact],
        "reason": (
            f"Mission access status is '{actual_status}', not '{expected_status}'."
        ),
    }
