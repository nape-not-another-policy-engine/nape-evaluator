def evaluate(evidence, evaluations, metadata):
    ...
    approval_evaluation = evaluation_index.get("data_export_approval_present")
    if approval_evaluation is None:
        return _build_inconclusive_result(
            "This test requires a data_export_approval_present evaluation with an equals criterion."
        )

    expected_value = _read_boolean_equals(approval_evaluation)
    if expected_value is None:
        return _build_inconclusive_result(
            "This test requires data_export_approval_present criteria.equals to be boolean."
        )
    ...
    if approval_fact["status"] != "found":
        return _build_inconclusive_result(
            [approval_fact],
            "Unable to evaluate because the data_export_approval_present fact could not be established.",
        )
...
def _extract_approval_fact(evidence):
    value = evidence.get("patient_data_export", {}).get("data_export_approval_present")
    if value is None:
        return {
...
def _evaluate_equals(approval_fact, expected_value):
    ...
    if actual == expected_value:
        return {
            ...
            "facts": [approval_fact],
            "reason": (
                f"data_export_approval_present is {actual}, which matches the expected value."
            ),
        }
    return {
        "conclusion": "false",
        "facts": [approval_fact],
        "reason": (
            f"data_export_approval_present is {actual}, which does not match the expected value {expected_value}."
        ),
    }
