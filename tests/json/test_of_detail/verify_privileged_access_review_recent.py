...
def evaluate(evidence, evaluations, metadata):
    ...
    if timestamp_fact["status"] != "found":
        return _build_inconclusive_result(
            [timestamp_fact],
            "Unable to evaluate because the privileged access last_review_timestamp fact could not be established.",
        )
...
def _evaluate_minimum_timestamp(timestamp_fact, minimum_timestamp):
    ...
    if actual_timestamp >= minimum_timestamp:
        return {
            ...
            "reason": (
                f"Privileged access last_review_timestamp is {timestamp_fact['value']}, "
                "which meets the required minimum datetime."
            ),
        }
    return {
        "conclusion": "false",
        ...
        "reason": (
            f"Privileged access last_review_timestamp is {timestamp_fact['value']}, "
            "which is older than the required minimum datetime."
        ),
    }
