def evaluate(evidence, evaluations, metadata):
    ...
    if status_evaluation is None:
        return _build_error_result(
            "This test requires a release_status evaluation with a disallowed_values criterion."
        )

    disallowed_values = _read_disallowed_values(status_evaluation)
    if not disallowed_values:
        return _build_error_result(
            "This test requires release_status criteria.disallowed_values to be a non-empty text array."
        )
    ...
    return _evaluate_disallowed_values(status_fact, disallowed_values)
...
def _read_disallowed_values(evaluation):
    values = evaluation.get("criteria", {}).get("disallowed_values")
...
def _evaluate_disallowed_values(status_fact, disallowed_values):
    actual = status_fact["value"]
    if actual in disallowed_values:
        return {
            "conclusion": "false",
            "facts": [status_fact],
            "reason": f"release_status is '{actual}', which is in the disallowed value set.",
        }
    return {
        "conclusion": "true",
        "facts": [status_fact],
        "reason": f"release_status is '{actual}', which is not in the disallowed value set.",
    }
