def evaluate(evidence, evaluations, metadata):
    ...
    status_evaluation = evaluation_index.get("restore_drill_status")
    if status_evaluation is None:
        return _build_inconclusive_result(
            "This test requires a restore_drill_status evaluation with an equals criterion."
        )

    expected_status = _read_expected_status(status_evaluation)
    if expected_status is None:
        return _build_inconclusive_result(
            "This test requires restore_drill_status criteria.equals to be a text value."
        )
    ...
    if status_fact["status"] != "found":
        return {
            ...
            "facts": [status_fact],
            "reason": "The restore drill evidence did not provide a usable restore_drill_status.",
        }
...
def _extract_status_fact(evidence):
    status = evidence.get("restore_drill", {}).get("restore_drill_status")
    return {
...
def _evaluate_status(status_fact, expected_status):
    ...
    if actual_status == expected_status:
        return {
            "conclusion": "true",
            "facts": [status_fact],
            "reason": f"Restore drill status is {expected_status}.",
        }

    return {
        "conclusion": "false",
        "facts": [status_fact],
        "reason": f"Restore drill status is '{actual_status}', not '{expected_status}'.",
    }
