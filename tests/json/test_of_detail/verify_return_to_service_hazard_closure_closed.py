def evaluate(evidence, evaluations, metadata):
    ...
    status_evaluation = evaluation_index.get("hazard_closure_status")
    if status_evaluation is None:
        return _build_inconclusive_result(
            "This test requires a hazard_closure_status evaluation with an equals criterion."
        )

    expected_status = _read_expected_status(status_evaluation)
    if expected_status is None:
        return _build_inconclusive_result(
            "This test requires hazard_closure_status criteria.equals to be a text value."
        )
    ...
    if status_fact["status"] != "found":
        return {
            ...
            "facts": [status_fact],
            "reason": "The return-to-service evidence did not provide a usable hazard closure status.",
        }
...
def _evaluate_status(status_fact, expected_status):
    ...
    if actual_status == expected_status:
        return {
            "conclusion": "true",
            "facts": [status_fact],
            "reason": f"Hazard closure status is {expected_status}.",
        }
    ...
    return {
        ...
        "facts": [status_fact],
        "reason": (
            f"Hazard closure status is '{actual_status}', not '{expected_status}'."
        ),
    }
