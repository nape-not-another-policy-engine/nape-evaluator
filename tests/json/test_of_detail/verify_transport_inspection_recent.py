...
def evaluate(evidence, evaluations, metadata):
    ...
    timestamp_evaluation = evaluation_index.get("inspection_timestamp")
    if timestamp_evaluation is None:
        return _build_inconclusive_result(
            "This test requires an inspection_timestamp evaluation with a minimum criterion."
        )

    minimum_timestamp = _read_minimum_datetime(timestamp_evaluation)
    if minimum_timestamp is None:
        return _build_inconclusive_result(
            "This test requires inspection_timestamp criteria.minimum to be an ISO-8601 datetime string."
        )
    ...
    if timestamp_fact["status"] != "found":
        return _build_inconclusive_result(
            [timestamp_fact],
            "Unable to evaluate because the inspection_timestamp fact could not be established.",
        )
...
def _evaluate_minimum_timestamp(timestamp_fact, minimum_timestamp):
    ...
    if actual_timestamp >= minimum_timestamp:
        return {
            ...
            "reason": (
                f"inspection_timestamp is {timestamp_fact['value']}, which meets the required minimum datetime."
            ),
        }
    return {
        "conclusion": "false",
