def evaluate(evidence, evaluations, metadata):
    ...
    status_evaluation = evaluation_index.get("pre_trip_inspection_status")
    if status_evaluation is None:
        return _build_inconclusive_result(
            "This test requires a pre_trip_inspection_status evaluation with an equals criterion."
        )

    expected_status = _read_expected_status(status_evaluation)
    if expected_status is None:
        return _build_inconclusive_result(
            "This test requires pre_trip_inspection_status criteria.equals to be a text value."
        )
    ...
    if status_fact["status"] != "found":
        return {
            ...
            "facts": [status_fact],
            "reason": "The transportation evidence did not provide a usable inspection status.",
        }
...
def _extract_status_fact(evidence):
    status = evidence.get("vehicle_inspection", {}).get("inspection_status")
    return {
...
def _evaluate_status(status_fact, expected_status):
    ...
    if actual_status == expected_status:
        return {
            "conclusion": "true",
            "facts": [status_fact],
            "reason": f"Pre-trip inspection status is {expected_status}.",
        }

    return {
        "conclusion": "false",
        "facts": [status_fact],
        "reason": f"Pre-trip inspection status is '{actual_status}', not '{expected_status}'.",
    }
