NAMESPACE = {"ns": "https://example.com/author"}
...
def evaluate(evidence, evaluations, metadata):
    ...
    if status_fact["status"] != "found":
        return {
            ...
            "facts": [status_fact],
            "reason": "The XML namespace-aware lookup did not establish a usable status value.",
        }
...
def _validate_metadata(metadata, evidence):
    ...
    if getattr(evidence, "tag", None) is None:
        return _build_inconclusive_result("This test expects an XML root element.")
    return None
...
def _extract_status_fact(evidence):
    status_text = evidence.findtext("ns:status", namespaces=NAMESPACE)
...
def _evaluate_status(status_fact, expected_status):
    ...
    if actual_status == expected_status:
        return {
            "conclusion": "true",
            "facts": [status_fact],
            "reason": f"The namespace-aware XML status is {expected_status}.",
        }
    ...
    return {
        ...
        "facts": [status_fact],
        "reason": (
            f"The namespace-aware XML status is '{actual_status}', not '{expected_status}'."
        ),
    }
